...
# url: str = "https://mydss.mo.gov/healthcare/seniors-disabled/"
url: str = "https://mydss.mo.gov/healthcare/seniors-disabled/"
...
# Print the first 10 words and the total number of words
print(words[:10])
print(len(words))
...
clean_words = [w.strip(".,:;!?()[]\"' ") for w in words if len(w) > 5]
...
# Focus on the 5 most common words for a simple bar chart.
...
markerline, stemlines, baseline = plt.stem(top_df["word"], top_df["len"])
markerline.set_color("yellow")  # Dot color
stemlines.set_color("orange")  # line color
